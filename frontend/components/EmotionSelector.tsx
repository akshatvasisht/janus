...
export default function EmotionSelector({ value, onChange }: EmotionSelectorProps) {
  const options: EmotionOverride[] = ['auto', 'calm', 'urgent'];
  ...
  return (
    <div className="flex gap-2">
      {options.map((opt) => {
        ...
        if (isActive) {
            if (opt === 'urgent') activeClass = 'bg-red-900/50 border-red-500 text-red-100';
            else if (opt === 'calm') activeClass = 'bg-cyan-900/50 border-cyan-500 text-cyan-100';
